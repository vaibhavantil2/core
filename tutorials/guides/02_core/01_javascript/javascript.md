...
Setting up a Main application is just as simple as calling a function. First, reference the Glue42 [Web Platform](https://www.npmjs.com/package/@glue42/web-platform) script in the **Clients** app and then initialize the library. The Web Platform library handles the entire Glue42 environment, which is necessary for the [Web Client](../../../core/core-concepts/web-client/overview/index.html) apps to be able to connect to the Main application and to each other.
...
After refreshing the app, you should see in the top left corner that Glue42 is available. This means that you have successfully initialized the [Main Application](../../../core/core-concepts/web-platform/overview/index.html).
...
Instruct the **Clients** app to open the **Stocks** app in a new window when the user clicks on the "Stocks" button. Locate the `TODO: Chapter 3.1` comment inside the `stocksButtonHandler()` function. Use the [`open()`](../../../core/capabilities/window-management/index.html#!API-open) method to open the **Stocks** app in a new window. The `instanceID` and `counter` variables ensure that the name of each new **Stocks** instance will be unique:
...
Go to the **Stocks** app and find the `TODO: Chapter 3.1` comment in the `stockClickedHandler()` function. Currently, it rewrites the value of `window.location.href` to redirect to the **Stock Details** view. Remove that and use the [`open()`](../../../reference/core/latest/windows/index.html#!API-open) method instead. Use the [`list()`](../../../core/capabilities/window-management/index.html#!API-list) method to get a collection of all Glue42 Windows and check whether the clicked stock is already open in a window. It is safe to search by `name`, because all Glue42 Window instances must have a unique `name` property:
...
*Note that each new instance of the **Stocks** app will create a new stream instance. In real world scenarios, this should be handled differently - e.g., by a system app acting as a designated data provider. For more details, see [Plugins](../../../core/capabilities/plugins/index.html).*
...
The **Clients** app is the [Main Application](../../../core/core-concepts/web-platform/overview/index.html) and besides the `GlueWorkspaces()` factory function, its configuration object requires also a `workspaces` property defining where the Workspaces App is located.
