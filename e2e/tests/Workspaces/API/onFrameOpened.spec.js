describe('onFrameOpened() ', () => {
    ...
    it('a new frame window should exist when notified of a new frame', (done) => {
        const ready = gtf.waitFor(2, done);

        glue.workspaces
            .onFrameOpened(() => {
                try {
                    const hasFrameWindow = glue.windows.list().some((win) => win.name === "Workspaces");
                    expect(hasFrameWindow).to.be.true;
                    ready();
                } catch (error) {
                    done(error);
                }
