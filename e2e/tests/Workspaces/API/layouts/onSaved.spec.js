describe('layouts.onSaved ', () => {
    ...
    it('should notify when a new layout is imported', (done) => {
        ...
        glue.workspaces.layouts.onSaved(ready)
            .then((unSub) => {
                ...
                return glue.workspaces.layouts.import([defaultLayout]);
            })
            .then(ready)
            .catch(done);
    });
    ...
    it('should notify when an existing layout is imported', (done) => {
        ...
        glue.workspaces.layouts.onSaved(ready)
            ...
            .then(() => glue.workspaces.layouts.import([defaultLayoutModified]))
            .then(ready)
            .catch(done);
    });
    ...
    it('should notify twice when two layouts are imported (sequentially)', (done) => {
        ...
        glue.workspaces.layouts.onSaved(ready)
            ...
            .then(() => glue.workspaces.layouts.import([secondLayout]))
            .then(ready)
            .catch(done);
    });
    ...
    it('should notify twice when two layouts are imported (parallel - promise.all)', (done) => {
        ...
        glue.workspaces.layouts.onSaved(ready)
            .then((unSub) => {
                unSubFuncs.push(unSub);
                return Promise.all([
                    glue.workspaces.layouts.import([secondLayout]),
                    glue.workspaces.layouts.import([defaultLayout])
                ]);
            })
            .then(ready)
            .catch(done);
    ...
    it('should notify twice when two layouts are imported (parallel - same array)', (done) => {
        ...
        glue.workspaces.layouts.onSaved(ready)
            .then((unSub) => {
                unSubFuncs.push(unSub);
                return glue.workspaces.layouts.import([defaultLayout, secondLayout]);
            })
            .then(ready)
            .catch(done);
    });
    ...
    it('should notify twice when two layouts are imported (on notify)', (done) => {
        ...
        glue.workspaces.layouts.onSaved(() => {
            if (!called) {
                called = true;
                glue.workspaces.layouts.import([secondLayout]).then(ready).catch(done);
            }
            ready();
        })
            .then((unSub) => {
